...
class RoleEntityRepositoryImpl(
    ...
    private fun readRights(obj: PGobject): RoleEntity.Rights =
        objectMapper.readValue(obj.value, RoleEntity.Rights::class.java)
    ...
    override fun save(entity: RoleEntity): RoleEntity =
        npJdbc.queryForObject(
            """
                insert into role
                (created_at, updated_at, community_id, name, priority, rights)
                values
                (:created_at, :updated_at, :community_id, :name, :priority, :rights)
                returning *;
            """.trimIndent(),
